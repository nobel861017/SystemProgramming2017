...
void csiebox_server_init(csiebox_server** server, int argc, char** argv) {
	...
	*server = tmp;
    ...
    sprintf(path_name, "%s/csiebox_server.pid", tmp->arg.run_path);
    int pid_file_fd = open(path_name, O_WRONLY | O_TRUNC);
    char pid_str[PATH_MAX];
    sprintf(pid_str, "%d", getpid());
    write(pid_file_fd, pid_str, strlen(pid_str));
    close(pid_file_fd);
}
void sign_func(int sig){
	if(sig == SIGUSR1){
		//The htonl() function converts the unsigned integer hostlong from host byte order to network byte order.
		uint32_t active_thread_num_32_t = htonl(active_thread_num);
		write(fifo_fd, &active_thread_num_32_t, sizeof(uint32_t));
	}
...
int csiebox_server_run(csiebox_server* server) {
	...
	//----------- Active Threads Statics -------
	char path[PATH_MAX];
	memset(path, 0, sizeof(path));
	pid_t cur_pid = getpid();
	sprintf(path, "%s/fifo.%d", server->arg.run_path, cur_pid);
	//puts(path);
	strcpy(fifo_path, path);
	mkfifo(path, 0644);
	fifo_fd = open(path, O_RDWR);
	signal(SIGUSR1, sign_func);
	signal(SIGINT, sign_func);
	signal(SIGTERM, sign_func);
	//------------------------------------------
