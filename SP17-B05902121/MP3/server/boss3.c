    ...
    while (1){
        ...
        for(int i = 0; i < config.num_miners; i++){
            if(FD_ISSET(client_fds[i].input_fd, &working_readset)){
                memset(&message[i], 0, sizeof(Protocol));
                read(client_fds[i].input_fd, &message[i], sizeof(Protocol));
                fprintf(stderr, "Got message from %d\n", client_fds[i].input_fd);
            }
        }
        for(int i = 0; i < config.num_miners; i++){
            if(FD_ISSET(client_fds[i].input_fd, &working_readset)){
                ...
                if(message[i].op == FOUND_TREASURE){
                    ...
                    strcpy(Found.md5, message[i].md5);
                    for(int k = 0; k < config.num_miners; k++){
                        if(i == k){
                            Protocol con;
                            memset(&con, 0, sizeof(Protocol));
                            con.op = CONTINUE_WORK;
                            fprintf(stderr, "%d Found treasure\n",client_fds[k].output_fd);
                            write(client_fds[k].output_fd, &con, sizeof(con));
                ...
                else if(message[i].op == CLIENT_CANNOT_FIND_TREASURE){
