...
Protocol Get_multiple_Protocol_from_server(int input_fd, MD5_CTX miner_ans){
...
void Check_Prefix_Zeros(int input_fd, int output_fd, MD5_CTX miner_ans, int *done, int n_treasure, int depth, char *mine_str, char *name){
    ...
    if(valid){
        ...
        *done = 1;
        ...
        timeout.tv_sec = 0;
        timeout.tv_usec = 10;
        message = Get_multiple_Protocol_from_server(input_fd, miner_ans);
...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        while(1){
            /*if(flag2){
                fprintf(stderr, "in second while(1) loop\n");
                flag2 = 0;
            }*/
            Protocol dig_mine;
            memset(&dig_mine, 0, sizeof(dig_mine));
            dig_mine = Get_multiple_Protocol_from_server(input_fd, miner_ans);
            if(dig_mine.op == MINE){
                MD5_Init(&miner_ans);
            ...
            Protocol Found;
            memset(&Found, 0, sizeof(Found));
            Found = Get_multiple_Protocol_from_server(input_fd, miner_ans);
            if(Found.op == FOUND_TREASURE){
                MD5_CTX tmp;
                MD5_Init(&tmp);
                tmp = Found.c;
                //tmp = miner_ans;
                /*for(int i = 0; i < Found.strlength; i++){
                    md5_add(&tmp, (int)Found.append_str[i]);
                }*/
                printf("while(1): ");
                print_md5(tmp);
            }
